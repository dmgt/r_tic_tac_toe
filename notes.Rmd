...
test_numeric_move <- function(next_move) {
  ...
  # First check if input is numeric and 1 - 9
  if (next_move %in% c(1,2,3,4,5,6,7,8,9)) { 
    need_reenter1 <- F
  } else { 
      need_reenter1 <- T
      print(glue("It looks like you entered '{next_move}', which is not a position on the board.
                    Please try again and enter a whole number between 1 and 9"))
        }
  
  # Second, check if the numeric value has already been played elsewhere on the board (numbers can't overlap)
  if (next_move %in% player_1_moves | next_move %in% player_1_moves) {
      need_reenter2 <- T
  ...
  need_reenter <- ifelse(need_reenter1 == T | need_reenter2 == T, T, F)
  ...
  #### While loop with next moves by both players, including input checking ####
   ...
   while (results$continue_play == T ){
      ...
      # Check content of input, and re-prompt player from start of loop if needed
           need_reenter <- test_numeric_move(p1_next_move <- player1_inputs$n)
           if ( need_reenter == T ) {
             next
      ...
      player2_inputs <- midgame_prompt(player_number = 2)
       ...
       # Check content of input, and re-prompt player if needed
           need_reenter <- test_numeric_move(p2_next_move <- player2_inputs$n)
           if ( need_reenter == T ) {
             next
           }else if (need_reenter == F) {
             player_2_moves <- append(player_2_moves, p2_next_move)
