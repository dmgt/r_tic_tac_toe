...
initial_prompt <- function()  {
  ...
  print(glue("
             ...
             There are a total of 9 positions on the board, as shown below:
  ...
  print(glue("Player 1 will go first. Your plays will use the 'x' symbol. Player 2 is 'o'.
 ...
 player2_initial_prompt <- function(){
    n <- (readline("Player 2, it's time for your first move. Please enter a number from 1-9: "))
    n <- as.numeric(n)
...
midgame_prompt <- function(player_number){
    n <- (readline(glue("Player {player_number}, what is your next move? Please enter a number from 1-9: ")))
    n <- as.numeric(n)
...
tic_tac_toe <- function ()  {
   ...
   player_2_moves <- inputs2$n
   
   plot_board(player_1_moves, player_2_moves)
   
   
   #### Loop with next moves by both players ####
      ...
      evaluate_win_conditions(player_1_moves, player_2_moves) 
...
if (max(p1_results) == 3 ) {
  ...
  if (continue_play == F) print("Congrats {winner}, you won!") 
...
# is.element(c(1,2,3), c(5,3,2,1)) %>% sum()  <- get this working in list comprehension

# List of vectors of winning sets
list_of_winning_moves <- list(c(1,2,3), c(4,5,6), c(7,8,9), c(1,4,7), c(2,5,8), c(3,6,9), c(1,5,9), c(3,5,7))

example_moves <- c(1,2,3,4)

# Test manual instance of list comprehension - correct
is.element(unlist(list_of_winning_moves[1]), example_moves) %>% sum() 

# Test examples that should fail - yes, works as expected
is.element(unlist(list_of_winning_moves[1]), c(1,2)) %>% sum()    # too short
is.element(unlist(list_of_winning_moves[1]), c(1,2,9)) %>% sum()  # 3 plays, but not in right positions

# Map examples
# Extract the third element of each list
map(list_of_winning_moves, 3 ) 

# Map return a list, map_int simplifies the output to a vector of integers
results <- map_int(list_of_winning_moves, function(x) is.element(unlist(x), example_moves) %>% sum() )  
