...
```{r setup}
library(glue)
library(tidyverse) # see if can import more specific library
```
...
initial_prompt <- function()  {
 ...
 player2_initial_prompt <- function(){
    n <- (readline("
    Nice! Player 1, your 'x' is now on the board. 
                   
    Player 2, it's time for your first move with 'o'. Please enter a number from 1-9: "))
    n <- as.numeric(n)
...
evaluate_win_conditions <-function(player_1_moves, player_2_moves) {
  ...
  # Starting condition (will switch to F when one player has won game)
  continue_play <- T
...
if (max(p1_results) == 3 ) {
 ...
 results <- list(continue_play, winner)
...
tic_tac_toe <- function ()  {
   ...
   #Initialize - list of 1) Variable if play shoul continue, 2) Character string of winner
   results <- list(T, NA)
